...
MY_IP=""
MY_PATH=$(dirname "$0")
ARTIFACT_PATH="$MY_PATH/../../images/com.wattsinnovations.auterion_os_1.3.6.auterionos"
SERIAL=""
...
# disable wifi here, test on skynode if --set wifiEnabled=0 turns off wifi
disable_wifi () {
...
# Copy the appropriate override.env file
scp_override_env () {
	if ! [ -f ../tools/stock_override/override.env ]; then
		echo "${RedText}Failed!" 
		echo "override.env file missing from the ../tools/stock_override directory. Add the appropriate file and try again"
		fail
	fi
	
	sshpass -p ${TARGET_PASSWORD} scp $SSH_OPTS ../tools/stock_override/override.env ${TARGET_USER}@${TARGET_IP}:../data
...
welcome_to_watts () {
	...
	echo
	echo "WATTS UPDATER - STOCK P-SKY"
}
