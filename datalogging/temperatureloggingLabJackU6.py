...
if __name__ == '__main__':
    ...
    logPath = os.getcwd() + "\\temperature_log" + timestamp+".csv"

    print("########################################################")
    print("Temperature logging script using LabJack U6 and K-Type Thermocouple connected to AIN0")
    print("\nSaving data at " + str(loggingRateHz) + "Hz")
    print("\nLog file: " + logPath)
    print("\nPress Ctrl+C at any point to stop logging and close logfile.\n")
    print("\n########################################################")
    ...
    with open(logPath, mode='w', newline='') as logFile:
        ...
        try:
            while True:
                ...
                print(timestamp + "\t\t" + "%s" % temp)

                logWriter.writerow([timestamp, temp])
                time.sleep(hz_to_sec(loggingRateHz))
                pass

        except KeyboardInterrupt:
            print("\nStopping...")
            print("Logfile: " + logPath + " saved.")
