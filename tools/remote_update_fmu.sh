...
if [ "$1" == "-h" ] || [ "$1" == "--help" ] || [ $# -lt 2 ]; then
	echo "Usage: $0 -f <firmware.px4|.elf> [-c <configuration_dir>] -d <IP/Device> [-u <user>] [-p <ssh_port>] [-i <ssh_key>]  [-o <options>]"
	exit 1
fi
...
while getopts ":f:c:d:p:u:i:o:" opt; do
	case ${opt} in
		f )
			if [ -n "$OPTARG" ]; then
...
$tar_cmd -C "$tmp_dir" --sort=name --owner=root:0 --group=root:0 --mtime='2019-01-01 00:00:00' -cvf $target_file_name $firmware_path $config_path
scp -P $ssh_port $ssh_key $options "$target_file_name" $ssh_user@"$device":$target_dir
popd &>/dev/null
rm -rf "$tmp_dir"

# grab status output
cmd="tail --follow=name $target_dir/update_status 2>/dev/null || true"
ssh $ssh_key $options -t -p $ssh_port $ssh_user@$device "$cmd"
